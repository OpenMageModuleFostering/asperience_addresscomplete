...
SearchZipCode.prototype = {
	initialize : function(form, fPostcode, fCity, fRegionId, fCountry, img, destinationElement, emptyText, urlCountry, urlPostCode, urlRegion, alert, adminAddress ){
		this.form   = $(form);
		...
		Event.observe(this.fPostcode, 'change', this.cityFocus.bind(this));
		if(this.fCity)
			Event.observe(this.fCity,	  'focus',  this.cityFocus.bind(this));
	...
	countryInit: function(event) {
		this.addressAutocompleter.setParameters('country_id=' + this.fCountry.value);
		if (this.fPostcode.value == this.emptyText
				|| this.fPostcode.value.length == 0 || this.fRegionId.value == '' || this.fRegionId.value == 0 ) {
			this.img.show();
	...
	blur : function(event) {
		this.img.hide();
		if(this.fPostcode.value == '') {
			this.fPostcode.value=this.emptyText;
		}
	},
	
	keyup : function(event) {
		if (this.adminAddress == false && this.fRegionId.disabled == false) {
			this.fRegionId.disabled = true;
			this.fRegionId.parentNode.parentNode.hide();
		}
	...
	completeCountry : function(transport) {
		...
		if (result.region_count == 1) {
			this.fRegionId.selectedIndex = 1;
		}
...
var AddressAutocompleter = Class.create(Ajax.Autocompleter, {
	...
	onComplete: function(request) {
		...
		if(this.searchZipCode.adminAddress == false) {
...
GoogleAddressAutocomplete.prototype = {
	initialize: function(fPostcode, fCity, fRegionId, fRegion, fCountry, img , fStreet1, fStreet2, fFormattedAddress, fLat, fLong, fUrl, searchZipCode, adminAddress){
		...
		if(this.searchZipCode.form != null) {
			// For non admin screens
			Event.observe(this.searchZipCode.form,'submit', this.submit.bind(this));
		}
	},
...
//				if(count==3) this.regionName = removeDiacritics(this.neighborhood);
	...
	submit : function(event) {
		for (var f in this.formFieldsValue) {
			if(this.adminAddress == false && $(f) != null) {
				$(f).disabled = false;
			}
		}
		return true;
